...
export default function CompleteProfile() {
    ...
    // 🚀 Orqaga qaytish va tokenni tozalash
    const handleBack = () => {
        try {
            logout();
        } catch (e) {
            console.error("Tokenni tozalashda xatolik:", e);
        }
        navigate("/");
    };
    ...
    return (
      ...
      `}</style>
            ...
            <main className="flex-1 flex items-center justify-center p-4 relative z-10 py-10">
                <div className="w-full max-w-[480px] bg-black/40 backdrop-blur-2xl border border-white/20 shadow-[0_30px_60px_rgba(0,0,0,0.6)] rounded-[2.5rem] p-8 sm:p-10 relative">

                    {/* 🚀 KARTA ICHIDAGI ORQAGA QAYTISH TUGMASI */}
                    <button
                        type="button"
                        onClick={handleBack}
                        className="absolute top-6 left-6 w-10 h-10 flex items-center justify-center rounded-full bg-white/5 hover:bg-white/10 text-gray-300 hover:text-white transition-colors border border-white/10 z-20"
                        title="Chiqish va Orqaga qaytish"
                    >
                        <ChevronLeft size={22} />
                    </button>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-5">
                        ...
                        {/* VILOYAT */}
                        <div className={`relative ${activeDropdown === 'region' ? 'z-50' : 'z-20'}`}>
                            <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1 mb-2 block">
                                Viloyat
                            </label>
                            <div
                                onClick={() => {
                                    if (!loading) setActiveDropdown(activeDropdown === 'region' ? null : 'region');
                                }}
                            ...
                            {activeDropdown === 'region' && (
                                <>
                                    ...
                                    <div className="absolute top-[105%] left-0 w-full bg-gray-900/95 backdrop-blur-xl border border-white/20 rounded-2xl shadow-2xl overflow-hidden flex flex-col z-50">
                                        <div className="max-h-[220px] overflow-y-auto custom-scroll relative z-50">
                        ...
                        {/* TUMAN */}
                        {formData.region && (
                            <div className={`relative animate-in fade-in slide-in-from-top-2 duration-300 ${activeDropdown === 'district' ? 'z-50' : 'z-20'}`}>
                                <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1 mb-2 block">
                                    Tuman (Shahar)
                                </label>
                                <div
                                    onClick={() => {
                                        if (!loading) setActiveDropdown(activeDropdown === 'district' ? null : 'district');
                                    }}
                        ...
                        <div className="pt-4 relative z-0">
                            <button
                                type="submit"
                                disabled={loading}
                                className={`w-full py-4 rounded-xl flex items-center justify-center gap-2 text-base font-bold transition-all duration-300 border ${loading
