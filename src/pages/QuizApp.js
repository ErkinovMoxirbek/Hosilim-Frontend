...
const Icons = {
  Check: () => (
    <svg className="w-6 h-6" fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24" strokeLinecap="round" strokeLinejoin="round"><path d="M20 6L9 17l-5-5" /></svg>
  ),
  X: () => (
    <svg className="w-6 h-6" fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24" strokeLinecap="round" strokeLinejoin="round"><path d="M18 6L6 18M6 6l12 12" /></svg>
  ),
  ArrowRight: () => (
    <svg className="w-5 h-5 ml-2" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24" strokeLinecap="round" strokeLinejoin="round"><path d="M13 7l5 5m0 0l-5 5m5-5H6" /></svg>
  ),
  Refresh: () => (
    <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24" strokeLinecap="round" strokeLinejoin="round"><path d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>
  ),
  Home: () => (
    <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24" strokeLinecap="round" strokeLinejoin="round"><path d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" /></svg>
  ),
  Sparkles: () => (
    <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z" /></svg>
  )
};
...
const QuizApp = () => {
  ...
  const [currentGroup, setCurrentGroup] = useState(null); // 'random' bo'lishi ham mumkin
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/quiz/groups`)
      ...
      .catch((e) => alive && setError("Internetni tekshiring, guruhlar yuklanmadi 🌸"))
      .finally(() => alive && setLoadingGroups(false));
    return () => { alive = false; };
  }, []);

  // Oddiy guruhni boshlash
  const startQuiz = (groupId) => {
    ...
    fetch(`${API_BASE}/quiz/questions/${groupId}`)
      .then((res) => res.json())
      .then((data) => {
        const processed = processQuestionsData(data); // Aralashtirish
  ...
  // RANDOM 25 (Super Mode)
  const startRandom25 = async () => {
    ...
    try {
      // Barcha guruhlardan emas, tasodifiy 3-4 ta guruhdan olamiz (serverni qiynamaslik uchun)
      // yoki barchasini olish ham mumkin agar guruhlar kam bo'lsa.
      // Keling, 5 ta tasodifiy guruhni tanlab olamiz:
      const randomGroups = shuffleArray(groups).slice(0, 5);
      
      // Barcha tanlangan guruhlarning savollarini parallel yuklaymiz
      const promises = randomGroups.map(g => 
        fetch(`${API_BASE}/quiz/questions/${g.id}`).then(res => res.json())
      );

      const results = await Promise.all(promises);
      
      // Hamma savollarni bitta massivga yig'amiz
      let allQuestions = results.flat();

      // Aralashtiramiz va variantlarni ham chalkashtiramiz
      let processed = processQuestionsData(allQuestions);

      // Faqat 25 tasini olamiz
    ...
    return (
      <Layout>
        <div className="p-8 bg-white/80 backdrop-blur rounded-3xl shadow-xl border border-red-100 text-center animate-fade-in-up">
          ...
          <button onClick={() => window.location.reload()} className="px-8 py-3 bg-red-400 text-white rounded-2xl font-bold shadow-lg shadow-red-200 hover:bg-red-500 transition transform hover:-translate-y-1">
            Qayta urinish
          </button>
        </div>
      </Layout>
        ...
        <div className="animate-fade-in-up space-y-8">
          ...
          {loadingGroups ? (
            ...
            <>
              ...
              {/* RANDOM 25 TUGMASI - Pastda alohida ajralib turadi */}
                ...
                >
                  ...
                  <div className="relative z-10 flex items-center justify-center space-x-3 text-white">
                    ...
                    <div className="text-left">
                      ...
                      <div className="text-xs text-pink-100 font-medium opacity-90">Barcha mavzulardan aralash savollar</div>
                    </div>
                  </div>
                </button>
              </div>
      ...
      <Layout>
        <div className="bg-white/80 backdrop-blur-xl rounded-[2rem] shadow-2xl border border-white overflow-hidden animate-fade-in-up">
          <div className="p-8 text-center relative overflow-hidden">
            {/* Orqa fon bezagi */}
            <div className={`absolute top-0 left-0 w-full h-2 bg-gradient-to-r ${isSuccess ? 'from-green-400 to-emerald-500' : 'from-pink-400 to-red-500'}`}></div>
            
            <h2 className="text-2xl font-bold text-gray-800 mt-2 mb-1">Natijalar</h2>
            <p className="text-sm text-gray-500 mb-8">{isSuccess ? "Ajoyib natija! 😍" : "Yaxshi, lekin yana urinib ko'ring 😊"}</p>
            ...
            <div className="relative inline-flex items-center justify-center mb-6">
              <svg className="w-40 h-40 transform -rotate-90 filter drop-shadow-md">
                ...
                <circle cx="80" cy="80" r="70" stroke="currentColor" strokeWidth="10" fill="transparent" 
                  ...
                  strokeLinecap="round"
                  className={isSuccess ? "text-green-500 transition-all duration-1000" : "text-pink-500 transition-all duration-1000"} 
                />
              </svg>
          ...
          <div className="p-6 bg-gray-50/50 flex flex-col sm:flex-row gap-3">
            <button onClick={resetGroup} className="flex-1 flex items-center justify-center py-4 px-6 bg-gray-800 text-white font-bold rounded-2xl shadow-lg hover:bg-gray-900 transition transform active:scale-95">
              <Icons.Refresh /> Qayta ishlash
            </button>
            <button onClick={backToGroups} className="flex-1 flex items-center justify-center py-4 px-6 bg-white text-gray-700 font-bold rounded-2xl shadow-md hover:bg-gray-50 border border-gray-100 transition transform active:scale-95">
          ...
          {wrongList.length > 0 && (
            <div className="p-6 bg-pink-50/30 border-t border-pink-100">
              <h3 className="text-lg font-bold text-gray-800 mb-4 pl-1 border-l-4 border-pink-400">Xatolar tahlili</h3>
  ...
  return (
    <Layout>
      ...
      <div className="flex items-center justify-between mb-6 px-2">
        <button onClick={backToGroups} className="w-10 h-10 flex items-center justify-center rounded-full bg-white shadow-sm text-gray-400 hover:text-gray-600 transition hover:bg-gray-50">
        ...
        <div className="bg-white rounded-[2rem] shadow-xl shadow-purple-100/50 border border-white overflow-hidden animate-fade-in-up">
          ...
          <div className="p-6 space-y-3">
            {currentQuestion.options.map((opt, i) => {
              ...
              let cardStyle = "bg-white border-2 border-gray-100 text-gray-600 hover:border-purple-200 hover:bg-purple-50";
              let icon = <div className="w-6 h-6 rounded-full border-2 border-gray-200"></div>; // Bo'sh aylana
              ...
              if (feedback) {
                if (selected && feedback.isCorrect) {
                  ...
                  icon = <div className="bg-white text-green-500 rounded-full p-0.5"><Icons.Check /></div>;
                } else if (selected && !feedback.isCorrect) {
                  cardStyle = "bg-red-500 border-red-500 text-white shadow-lg shadow-red-200";
                  icon = <div className="bg-white text-red-500 rounded-full p-0.5"><Icons.X /></div>;
                } else if (!feedback.isCorrect && correct) {
                  cardStyle = "bg-green-100 border-green-200 text-green-800";
                  icon = <div className="bg-green-500 text-white rounded-full p-0.5"><Icons.Check /></div>;
                } else {
              ...
              return (
                <button
                  ...
                  className={`w-full relative text-left p-4 rounded-2xl transition-all duration-300 font-medium flex items-center justify-between group ${cardStyle} ${!feedback ? 'active:scale-[0.98]' : ''}`}
                >
