...
async function fetchStatsWithFailover(signal) {
  ...
  for (const url of urls) {
    ...
    } catch { /* keyingisiga o‘tamiz */ }
  }
  return { totalUsers: 1200, totalTransactions: 4500, activeBrokers: 85 }; // Fallback real ko'rinish uchun
}
...
const Login = () => {
  ...
  useEffect(() => {
    ...
    fetchStatsWithFailover(ctrl.signal).then(s => setStats(s)).finally(() => setStatsLoading(false));
    return () => ctrl.abort();
  }, []);
  ...
  const handleSendOtp = async () => {
    if (!isValidUz(phone)) return setError("Telefon raqamini to'g'ri kiriting");
    setIsLoading(true);
    setError("");
    try {
    ...
    } catch { setError("OTP yuborishda xatolik yuz berdi"); }
    finally { setIsLoading(false); }
  };

  const handleVerifyOtp = async () => {
    if (otp.length !== 4) return setError("Kodni to'liq kiriting");
    setIsLoading(true);
    setError("");
    try {
      ...
      if (me.ok) {
        ...
        navigate(u?.data?.status === "ACTIVE" ? "/dashboard" : "/extra-info");
      } else navigate("/login");
    } catch { setError("Kod noto'g'ri kiritildi"); }
    finally { setIsLoading(false); }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col font-sans">
      ...
      <main className="flex-1 flex items-center justify-center p-4 relative overflow-hidden">
        {/* Orqa fon bezagi */}
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-green-100 rounded-full blur-[120px] opacity-30 pointer-events-none"></div>

        <div className="w-full max-w-5xl grid lg:grid-cols-2 gap-12 items-center relative z-10">
          
          {/* LEFT SIDE: Info (Faqat Desktop) */}
          ...
          <div className="bg-white rounded-[2rem] shadow-2xl shadow-green-100/50 border border-gray-100 p-8 md:p-10 relative">
            ...
            {step === 1 ? (
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-gray-500 mt-1 text-sm">Tizimga kirish uchun telefon raqamingizni kiriting</p>
                </div>
                ...
                <div className="space-y-4">
                  <div className="relative group">
                    <label className="text-xs font-bold text-gray-400 uppercase tracking-widest ml-1 mb-2 block">Telefon raqam</label>
                    <div className={`flex items-center bg-gray-50 border-2 rounded-2xl px-4 py-4 transition-all ${isValidUz(phone) ? 'border-green-500 bg-white' : 'border-gray-100 focus-within:border-green-600 focus-within:bg-white'}`}>
              ...
              <div className="space-y-6">
                <button onClick={() => setStep(1)} className="flex items-center gap-2 text-gray-400 hover:text-green-600 transition-colors text-sm font-semibold">
            ...
            <p className="mt-8 text-center text-xs text-gray-400 leading-relaxed">
              Kirish orqali siz Hosilim <span className="underline cursor-pointer">Xizmat ko'rsatish shartlari</span> va <span className="underline cursor-pointer">Maxfiylik siyosatiga</span> rozilik bildirasiz.
            </p>
          </div>
        </div>
      </main>
